 ...
 *           recipient = PRIV_<L>           (PrivateOwned, wallet-owned).
 * deshield: sender = PRIV_<L>              (PrivateOwned),
 *           recipient = ATA_<L> or HOLD_<L> (Public).
 * Writes the tx hash bytes into `out_tx_hash[32]`. */
int32_t ldex_token_shield(const char *config_path, const char *storage_path,
                          const uint8_t *sender, const uint8_t *recipient,
                          ldex_u128 amount, uint8_t *out_tx_hash);
int32_t ldex_token_deshield(const char *config_path, const char *storage_path,
                            const uint8_t *sender, const uint8_t *recipient,
                            ldex_u128 amount, uint8_t *out_tx_hash);
...
/* amm_v2 mode-1 PRIVATE add-liquidity. Replaces the legacy v1
 * `ldex_amm_private_add_liquidity` (which the chain now rejects as
 * InvalidPrivacyPreservingProof). All 3 user holdings must be
 * PrivateOwned wallet-owned accounts (PRIV_A/B/LP). The proof creates
 * the new commitment for PRIV_LP on first deposit. 7 accounts, no Clock.
 * Verified live tx 1cbadef0…2521496 at 24 min on Ryzen. */
int32_t ldex_amm_v2_private_add_liquidity(
    const char *config_path, const char *storage_path,
    const uint8_t *amm_v2_program_id,
    const uint8_t *user_holding_a, const uint8_t *user_holding_b,
    const uint8_t *user_holding_lp,
...
/* amm_v2 mode-1 PRIVATE remove-liquidity. Mirror of `_add_`. */
int32_t ldex_amm_v2_private_remove_liquidity(
    const char *config_path, const char *storage_path,
    const uint8_t *amm_v2_program_id,
    const uint8_t *user_holding_a, const uint8_t *user_holding_b,
    const uint8_t *user_holding_lp,
