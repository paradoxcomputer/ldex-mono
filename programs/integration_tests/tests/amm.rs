...
fn amm_new_definition_supports_all_fee_tiers() {
    ...
    let ta = Ids::token_a_definition();
    let tb = Ids::token_b_definition();
    for fees in [
        FEE_TIER_BPS_1,
        FEE_TIER_BPS_5,
        FEE_TIER_BPS_30,
        ...
        // Pool/vault/LP/lock PDAs are fee-tier-specific (RFP-004 Func #6:
        // pools for the same pair but different tiers coexist), so derive
        // this tier's ids and build the message against them.
        let pool_id = amm_core::compute_pool_pda(amm, ta, tb, fees);
        let vault_a_id = amm_core::compute_vault_pda(amm, pool_id, ta);
        let vault_b_id = amm_core::compute_vault_pda(amm, pool_id, tb);
        let lp_def_id = amm_core::compute_liquidity_token_pda(amm, pool_id);
        let lp_lock_id = amm_core::compute_lp_lock_holding_pda(amm, pool_id);

        let mut state = state_for_amm_tests_with_new_def();
        state.force_insert_account(vault_a_id, Accounts::vault_a_reinitializable());
        state.force_insert_account(vault_b_id, Accounts::vault_b_reinitializable());
        ...
        let pool_definition =
            PoolDefinition::try_from(&state.get_account_by_id(pool_id).data)
